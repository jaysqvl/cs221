...
void BinaryTree<T>::printLeftToRight(const Node* subRoot) const
{
    ...
    // Recursive definition (in-order-traversal)
...
void BinaryTree<T>::mirror(Node* subRoot)
{
    ...
    // moves down the left nodes and swaps their nodes
    mirror(subRoot->left);
    // moves down the right nodes and swaps their nodes
    mirror(subRoot->right);
    // all nodes should now have been traversed
}
...
void BinaryTree<T>::printPaths(const Node* subRoot, string path) const
{
    ...
    // Recursive definition (in-order-traversal)
    // left to right
    // print every node until NULL. when NULL, new line
...
bool BinaryTree<T>::isOrdered(const Node* subRoot) const
{
  // Your code here
  if (subRoot == NULL) {
    return true; // if we reach the end of a node return
  }
