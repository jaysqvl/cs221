...
template <typename T>
void printFlatTree(const vector<T>& elems, const string& name,
                   const string& description);
...
int main(int argc, char** argv)
{
    ...
    printFlatTree<int>({1,2,3,4,5,6,7}, "FLAT perfect",
                  "a perfect flat tree");

    printFlatTree<int>({1,2,3,4}, "FLAT complete",
                  "a complete flat tree");
    return 0;
}
                   ...
                   const string& description)
{
    ...
    cout << "printLeftRight: ";
    FlatTree<T> flat;
    flat.data.insert(flat.data.end(), elems.begin(), elems.end());
    flat.printLeftToRight();
}
