...
class ImgList {
  ...
  *********************************************************/
  private:
    // Constructor helpers
    ImgNode* makeRow(PNG& img, unsigned int y); // returns a fully constructed list
    void connectRow(ImgNode* head1, ImgNode* head2);
    
    // Clear helpers
    void deleteTopRow(ImgNode* head);

    // Render helpers
    void RenderSolid(PNG *& pngpointer) const;
    void RenderSolidAverage(PNG *& pngpointer) const;
    HSLAPixel * AverageGap(ImgNode* gapNode) const;

    void RenderGradient(PNG *& pngpointer) const;
    void RenderIgnoreFill(PNG *& pngpointer) const;
};
