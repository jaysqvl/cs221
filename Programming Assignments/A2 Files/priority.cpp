...
void PriorityNeighbours::Insert(PixelPoint p) {
  // complete your implementation below
  if(points.size() == 0 || isPixelCloser(p,points.at(0))) {
    points.insert(points.begin(),p);
    return;
  }

  for (int i = 1; i < points.size(); i++) {
    if (isPixelCloser(p,points.at(i))) {
...
bool PriorityNeighbours::isPixelCloser(PixelPoint pixel1, PixelPoint pixel2) {
  double pixel1_distance = pixel1.color.dist(refcolor);
  double pixel2_distance = pixel2.color.dist(refcolor);

  if (pixel1_distance < pixel2_distance) { // pixel1 distance strictly less than pixel2
    return true;
  } else if (pixel1_distance > pixel2_distance) { // pixel1 distance strictly greater than pixel 2
    return false;
  } else { // compare y's and x's
    if (pixel1.y < pixel2.y) { // minimal y distance return true
      return true;
    } else if (pixel1.y > pixel2.y) {
      return false;
    } else {
      if (pixel1.x < pixel2.x) { // minimal x distance return true
        return true;
      } else {
        return false;
      }
